...
export const HeaderButtonCart = () => {
  ...
  const FinalizePurchase = () => {
    cartItem.map((item) => {
      putUpQtyStockQuantity(
        item.id_product,
        StockItemQuantity(item.id_product) - UserItemQuantity(item.id_product)
      );
  ...
  return (
    <>
      <HeaderButtonCartCss onClick={() => MenuCart()}>
        <QuantityCartCss>
          {" "}
          {cartPriceSum > 0 ? cartItemSum : 0}{" "}
        </QuantityCartCss>
        <TiShoppingCart />
        <CartTotalPriceCss>
          {" "}
          {(cartPriceSum &&
            cartPriceSum.toLocaleString("pt-br", {
              style: "currency",
              currency: "BRL",
            })) ||
            0}{" "}
      ...
      <MenuCartCss variant={menuCart === true}>
        ...
        <ScrollCSS>
          {cartItem && cartItem.length > 0 ? (
                    ...
                    })}
                    {
                      <>
                        <ContainerGridCss>
                          <ButtonPutCSS>
                            <button
                              onClick={() => PutQuantity(item.id_product)}
                            >
                              {" "}
                              <MdAddShoppingCart />{" "}
                            </button>
            ...
            <EmptyCartCSS>
              <img
                src="https://programada.shopper.com.br/static/img/apple-icon.png"
                width={100}
              ></img>
